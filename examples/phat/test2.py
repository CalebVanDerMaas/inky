...
inky_display.set_border(inky_display.BLACK)  # Changed to BLACK
...
draw.rectangle((0, 0, inky_display.WIDTH, inky_display.HEIGHT), fill=1)  # Changed to 1
...
def draw_text(input_string, start_pos):
    ...
    for char in input_string.lower():
        if char in microFontDict:
            ...
            image.paste(0, current_pos, char_image)  # Changed to 0
...
draw.rectangle([0, 0, inky_display.WIDTH - 1, inky_display.HEIGHT - 1], outline=0)  # Changed to 0
draw.text((2, inky_display.HEIGHT - 10), f"Text start: {start_x},{start_y}", fill=0)  # Changed to 0
